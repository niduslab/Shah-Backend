        ...
        <div class="card mb-4">
            ...
            <header class="card-header">
                <div>
                    ...
                    <a href="javascript:void(0)" class="btn btn-md rounded font-sm" data-bs-toggle="modal" data-bs-target="#promotionModal" onclick="resetModal()"><strong>Add New Product</strong></a>
            ...
            if (file && file.type.startsWith('image/')) {
                ...
                reader.onload = function (e) {
                    ...
                    removeButton.onclick = function () {
                        ...
                        previewContainer.innerHTML = '';
                        renderPreviews(updatedFiles); // Re-render the remaining previews
                    };
...
function createFileList(files) {
    ...
    variationContainers.forEach(container => {
                    ...
                    variation.options.forEach(option => {
                        ...
                        optionCheckboxes.forEach(optionCheckbox => {
                            if (optionCheckbox.value === option.option) {
                                ...
                                let optionPrice = parseFloat(option.price);
                                if (optionPrice && typeof optionPrice !== 'string' && priceTag) {
                                    priceTag.textContent = `$${optionPrice}`; // Add price to span
       ...
       axios.get(`/admin/product/${productId}`)
        ...
        //         selector: '#description',
        //         setup: function(editor) {
        //             // When the editor is initialized, set the content
        //             editor.on('init', function() {
        //                 editor.setContent(product.description); // Set the content from your product
        //             });

        //             // Sync content to the textarea when the form is submitted
        //             document.getElementById('yourFormId').addEventListener('submit', function(e) {
        //                 e.preventDefault(); // Prevent default form submission if you need to handle it manually
        //                 tinymce.triggerSave(); // Sync the TinyMCE content to the textarea

        //                 const descriptionContent = tinymce.get('description').getContent(); // Get the TinyMCE content
        //                 console.log(descriptionContent); // Now this contains the content
        //                 // You can now submit the form or handle the data as needed
        //                 // e.g., using AJAX or traditional form submission

        //                 // Submit the form after handling content
        //                 this.submit();
        //             });
        //         }
        //     });
    ...
    // Remove spaces, hyphens, and special characters (&, etc.), then take the first 3 characters and convert to uppercase
    const categoryPrefix = categoryText.replace(/[\s-&]/g, '').substr(0, 3).toUpperCase();
    ...
    try {
        ...
        document.getElementById('image-preview-container').innerHTML = '';
            document.getElementById('slug_feedback').innerHTML = '';
            document.getElementById('char_count').innerHTML = '';
            document.getElementById('variation_values').innerHTML = '';
            document.getElementById('optionId').innerHTML = '';
            document.getElementById('variationOptionId').innerHTML = '';
            document.getElementById('product_id').innerHTML = '';
            document.getElementById('tags_json').innerHTML = '';
            document.getElementById('sku').innerHTML = '';
            document.getElementById('skuFeedback').innerHTML = '';
            document.getElementById('productForm').reset();
            const messageDiv = document.getElementById('show-error');
            messageDiv.innerHTML = '';
            ...
            variationContainers.forEach(container => {
                ...
                allPriceTags.forEach(priceTag => {
                    priceTag.textContent = ''; // Clear any previous prices
                    console.log("cleared previous prices");
                    $('.options-container').css('display', 'none');

                });
            });
            // $('.variation-value-tag').empty();
        }

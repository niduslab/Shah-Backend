...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (vendorShop.description && tinymce.get('shop_description')) {
        // let desc = JSON.parse(vendorShop.description);
        // shopDescriptionElement.value = desc || '';

        setTimeout(() => {tinymce.activeEditor.setContent(vendorShop.description)},1200);
    }
...
document.getElementById('vendorInfoForm').addEventListener('submit', function (e) {
    ...
    .then(data => {
        // Check if the expected 'message' field exists
        if (data.message) {
...
function vendorValidateForm() {
    const requiredFields = ['shopname', 'shop_email', 'phone', 'product_tax_percent', 'product_vat_percent', 'location', 'contact'];
    let isValid = true;
